...
## Current state
- **9+ commits, all pushed to `origin/main`** (`Thet9354/NTUSync`). Working tree clean. Latest: exam mode (SchemaV3), preceded by `f48429c` (B3 conflict detection + `.ics` export), `ebbf0d9`/`d2d405a` (13-week grid), `551bc60` (Phase 2) and `5942446` (Phase 1). Always confirm with `git log --oneline` + `git status` before assuming.
- **97 tests passing**, zero Swift warnings, builds clean for simulator and `generic/platform=iOS`.
...
- **Phase 3 (post-launch):** CloudKit public DB for community benches + occupancy check-ins (30-min TTL) — the only item that truly needs the paid membership — and compass-mode navigation (offline, no membership needed, but heading logic wants on-device verification; true AR geo-tracking is NOT available in Singapore).
